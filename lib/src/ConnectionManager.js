...
class Manager {
    ...
    checkConnections(rpc_user = "", rpc_password = "", resolve, reject) {
        const checkFunction = (resolve, reject) => {
            let fullList = (this.reducedUrls || this.urls).concat(this.url);
            ...
            fullList.forEach(url => {
                ...
                connectionPromises.push(() => {
                    ...
                    return conn.login(rpc_user, rpc_password).then((data) => {
                        let result = {[url]: new Date().getTime() - connectionStartTime};
            ...
            const sequentialPromise = connections =>
                connections.reduce(
                    (promise, connection) =>
                        promise.then(
                            result => connection().then(
            ...
            const connections = connectionPromises.map(
                cp => () => cp()
            );
