...
var Manager = function () {
    ...
    Manager.prototype.checkConnections = function checkConnections() {
        ...
        var checkFunction = function checkFunction(resolve, reject) {
            ...
            fullList.forEach(function (url) {
                ...
                connectionPromises.push(function () {
                    ...
                    return conn.login(rpc_user, rpc_password).then(function (data) {
                        ...
                        var result = (_result = {}, _result[url] = new Date().getTime() - connectionStartTime, _result);
            ...
            var sequentialPromise = function sequentialPromise(connections) {
                return connections.reduce(function (promise, connection) {
                    return promise.then(function (result) {
                        return connection().then(Array.prototype.concat.bind(result));
                    });
                }, Promise.resolve([]));
            };

            var connections = connectionPromises.map(function (cp) {
                return function () {
                    return cp();
                };
            });

            sequentialPromise(connections).then(function (res) {
